...
test('resumes real onboarding and finishes through durable quiz practice', async ({ page }) => {
  ...
  const generationStartedOrReady = page.getByText('Your quiz is being generated').or(page.getByText('coordination quiz is ready'));
  await expect(generationStartedOrReady).toBeVisible({ timeout: 15_000 });
