...
test('Plugins & models preloads once and only re-detects on manual refresh', async ({ page }) => {
  ...
  let pluginLoads = 0;
  await page.route('**/api/integrations', async route => { integrationLoads += 1; await route.continue(); });
  await page.route('**/api/v1/plugins', async route => { if (!route.request().url().includes('registry')) pluginLoads += 1; await route.continue(); });
  await page.reload();
