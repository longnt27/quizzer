...
const migrateLegacyEmbeddingProvider = (values, existingConfig = false) => {
  if (Object.hasOwn(values, 'embeddings.provider')) return values;
  const hasLegacyPlugin = Object.hasOwn(values, 'embeddings.embedderPlugin');
  if (!existingConfig && !hasLegacyPlugin) return values;
  const plugin = values['embeddings.embedderPlugin'];
  return {
    ...values,
    'embeddings.provider': typeof plugin === 'string' && plugin !== 'builtin' ? 'plugin' : 'ollama',
  };
};

export const writeUserSettings = async (appDataDirectory, values) => {
  const path = settingsPath(appDataDirectory);
  const existingConfig = await access(path).then(() => true).catch(error => {
    if (error?.code === 'ENOENT') return false;
    throw error;
  });
  const validated = migrateLegacyEmbeddingProvider(validateSettings(values), existingConfig);
