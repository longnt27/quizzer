...
Use `quizzer migrations list` to inspect migration status, hashes, and retained rollback paths. Use `quizzer backup create` for a separate consistent live SQLite backup. Source deployments can also stop Quizzer and copy the `.quizzer-data` directory. Set `QUIZZER_DATABASE_PATH` only when a custom database location is needed for a packaged or managed deployment.
