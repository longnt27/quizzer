...
> [!NOTE]
> **Public beta available:** `v1.0.0-beta.10` is ready to install. Downloads are protected by a signed manifest and SHA-256 checksums. The app does not yet have paid Apple notarization or a Windows publisher certificate, so those systems may show an unidentified-developer or unknown-publisher warning.
...
```sh
curl -fsSL https://github.com/longnt27/quizzer/releases/download/v1.0.0-beta.10/install.sh | sh
```

**Windows PowerShell**

```powershell
irm https://github.com/longnt27/quizzer/releases/download/v1.0.0-beta.10/install.ps1 | iex
```
