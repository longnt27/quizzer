...
The creation dialog closes immediately after saving the job. Each instance is assigned to a different test and makes one provider request at a time. Within that test, each question type is generated sequentially using the configured batch size. The default is 20, so 45 missing questions become requests of 20, 20, and 5; each later prompt can exclude everything accepted from earlier batches. Other instances work on other tests rather than generating overlapping candidates for the same test. In the desktop app, closing the window hides Quizzer in the tray so this work continues; the tray reports active and running job counts. Set **Continue in background** off in Settings when closing the last window should quit instead. If the local service is temporarily unreachable, Quizzer conservatively stays in the tray rather than risking an interrupted job.
...
After every request and validated round, Quizzer checkpoints progress, accepted questions, retry counters, and provider settings through the local service. The service accepts updates only from the worker holding the renewable 45-second lease, preventing an expired client from overwriting a resumed job. It also validates coverage plans, progress counters, provider-attempt history, and route changes against the immutable creation snapshot. Creating the final test and completing its job is one idempotent SQLite transaction. Remembered API credentials are decrypted by the desktop and handed directly to service memory after every service start; session credentials use the authenticated loopback channel. Values never enter jobs, settings, logs, backups, or diagnostics, and only configured provider names can be queried. A dropped connection moves the job into a waiting state and retries automatically when connectivity returns.
