    ...
    <Modal open title={`Ask AI about ${document.name}`} width={780} onCancel={close} footer={null} destroyOnHidden>
      <Space direction="vertical" size="middle" style={{ width: '100%' }}>
        <Space.Compact block>
          ...
          <Input value={model} onChange={event => setModel(event.target.value)} placeholder="Use provider default model" />
        </Space.Compact>
        ...
        <Input.TextArea value={question} onChange={event => setQuestion(event.target.value)} autoSize={{ minRows: 3, maxRows: 8 }}
          placeholder="What would you like to know?"
          onPressEnter={event => {
            if (!event.shiftKey) { event.preventDefault(); void submit(); }
          }} />
        <Button type="primary" icon={<SendOutlined />} loading={loading} disabled={!question.trim()} onClick={() => void submit()}>
          Ask AI
        </Button>
        <Typography.Text type="secondary">Press Enter to ask. Use Shift+Enter for a new line.</Typography.Text>
      </Space>
    </Modal>
  );
}
